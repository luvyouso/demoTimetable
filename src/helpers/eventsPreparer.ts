...
export const clusterizer = (events: any, minutes: any) => {
  ...
  minutes?.forEach(minute => {
    minute?.forEach(eventKey => {
      ...
      findNode.biggestGroupSize = Math.max(
        findNode.biggestGroupSize,
        minute?.length,
      );
...
export const prepareTimetable = (
  ...
  items?.forEach((item, index) => {
    ...
    let countedStartMinutes =
      day.start > originalStart ? 0 : getDayMinutes(originalStart);
    let endMinutes =
      day.end < originalEnd ? TOTAL_MINUTES : getDayMinutes(originalEnd);
    let countedEndMinutes =
      Math.max(endMinutes, countedStartMinutes + itemMinHeightInMinutes) - 1;
    ...
    for (
      let eventMinute = countedStartMinutes;
      eventMinute < countedEndMinutes;
      eventMinute++
    ) {
  ...
  timetable.clusters.forEach(cluster => {
    ...
    for (let nodeId in cluster.nodes) {
      maxGroupSize = Math.max(
        maxGroupSize,
        cluster.nodes[nodeId].biggestGroupSize,
      );
